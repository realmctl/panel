...
Realm Panel is powered by React, TypeScript, and Tailwind CSS using webpack at its core to generate compiled assets.
Release versions include pre-compiled, minified, and hashed assets ready-to-go.

If you are interested in running custom themes or making modifications to the React files you'll need a build
...
* [Go](https://golang.org/) 1.23.x or later (for Wings development)
...
The command above will download all of the dependencies necessary to get Realm assets building. After that, it is as
...
to the React files without having to reload the page you're on. To get started with this, run the command below.
