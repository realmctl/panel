...
async function getAllFilesFromEntry(entry: FileSystemEntry, path = ''): Promise<FileWithPath[]> {
    ...
    if (entry.isDirectory) {
        ...
        const results = await Promise.all(
            allEntries.map((e) => getAllFilesFromEntry(e, `${path}${dirEntry.name}/`))
        );
        return results.flat();
    }
...
export default ({ className }: WithClassname) => {
    ...
    const uploadFilesWithPaths = async (filesWithPaths: FileWithPath[]) => {
        ...
        // Create each directory using its immediate parent as root so Wings doesn't need to handle slashes in name.
        ...
        for (const subDir of dirsToCreate) {
            ...
            try {
                ...
                // Directory may already exist — only abort on a real failure by checking if it blocks uploads.
            }
        }
