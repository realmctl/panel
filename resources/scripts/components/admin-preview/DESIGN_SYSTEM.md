...
```
AdminPreviewContent          ← Page title (h1) + description
...
|-------|----------------|---------|
| `*Container.tsx` | Routing, flash key, `AdminPreviewContent` wrapper | `AdminPreviewUsersContainer.tsx` |
...
Import from `@/components/admin-preview/settings/settingsLayout` and `fieldClass.ts`.
...
From `@/components/admin-preview/settings/fieldClass`:
...
Use `TabNav` from `@/components/admin-preview/TabNav`.
...
Do **not** use large decorative icons in empty states. Sidebar nav icons are defined in `adminPreviewRoutes.ts` only.

---

## Overview page specifics

`AdminPreviewOverviewContainer.tsx` uses the same tokens but adds:
...
- [ ] Wrapped in `AdminPreviewContent` via container
...
- [ ] Links use `adminPreviewBasePath`
...
```
admin-preview/
  AdminPreview{Resource}Container.tsx   # routes + flash
  {resource}/
...
Register route in `adminPreviewRoutes.ts` and add to overview `managementSections` if appropriate.
